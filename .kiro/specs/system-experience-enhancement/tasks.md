...
- [x] 1. 基础设施：数据库表、Pydantic 模型与 TypeScript 类型定义
  - [x] 1.1 创建数据库迁移脚本，新增 `project_profiles`、`project_tags`、`project_tag_associations`、`stage_configs` 四张表
    - 在 `examples/web_ui_agent/backend/` 下创建 `migrations/` 目录，编写 SQL 迁移文件
    - 包含建表语句、索引和外键约束
    - _需求: 13.1, 14.1, 10.1_
  - [x] 1.2 在 `app/models/schemas.py` 中新增 Pydantic 模型
  ...
  - [x] 1.3 在 `frontend/src/types/index.ts` 中新增 TypeScript 类型
...
- [x] 2. 材料就绪状态 API 与评审就绪检查
  - [x] 2.1 在 `app/services/material_service.py` 中新增 `get_status` 方法
  ...
  - [x] 2.2 在 `app/routes/materials.py` 中新增 `GET /api/projects/{project_id}/materials/status` 端点
  ...
  - [x] 2.6 在 `frontend/src/services/api.ts` 中新增 `materialApi.status` 方法
    - 调用 `GET /api/projects/{project_id}/materials/status`
    - _需求: 1.3_
  - [x] 2.7 创建 `frontend/src/hooks/useReadinessChecker.ts` Hook
    - 页面加载时获取材料状态
...
- [x] 3. 名称映射 API 与标签解析器
  - [x] 3.1 在 `app/routes/competitions.py` 中新增 `GET /api/name-mappings` 端点
    - 直接返回 `rule_service.py` 中的 `COMPETITION_NAMES`、`TRACK_NAMES`、`GROUP_NAMES`
    - _需求: 7.5_
  - [x] 3.2 在 `frontend/src/services/api.ts` 中新增 `competitionApi.nameMappings` 方法
    - _需求: 7.5_
  - [x] 3.3 创建 `frontend/src/hooks/useLabelResolver.ts` Hook 和 `LabelResolverContext`
...
- [x] 5. 通用组件：BackButton 与全局错误处理
  - [x] 5.1 创建 `frontend/src/components/BackButton.tsx` 通用返回按钮组件
    - 接收 `to` 和 `label` 属性，渲染 `<Button type="text" icon={<ArrowLeftOutlined />}>`
    - 与 ProjectDashboard 已有返回按钮风格一致
    - _需求: 8.7_
  - [x] 5.2 增强 `frontend/src/services/api.ts` 中的 axios 响应拦截器
    - 4xx/5xx 错误统一通过 `message.error()` 展示
    - 存储失败请求配置，支持重试
    - 连续 3 次重试失败后显示兜底提示
    - _需求: 15.1, 15.4, 15.5_
  - [x] 5.3 创建 `frontend/src/components/NetworkStatusBar.tsx` 网络状态提示条
...
- [x] 6. 评审就绪前端集成：页面修改
  - [x] 6.1 修改 `TextReview.tsx`：集成 useReadinessChecker 和材料选择
  ...
  - [x] 6.2 修改 `OfflineReview.tsx`：集成 useReadinessChecker
    - 添加 BackButton（返回项目仪表盘）
  ...
  - [x] 6.3 创建 `frontend/src/components/ReviewSelectionDialog.tsx` 评审选择对话框
    - 在项目仪表盘中当多种评审类型均可用时弹出
    - 提供三个选项卡片：仅文本评审、仅离线路演评审、两者都评审
    - 点击后导航至对应页面
    - _需求: 6.4_
  - [x] 6.4 修改 `ProjectDashboard.tsx`：集成就绪状态与评审选择
    - 快捷操作卡片显示就绪状态标签（"就绪"/"材料未备齐"）
    - 未就绪卡片置灰并显示缺失材料提示
    - 多种评审均就绪时弹出 ReviewSelectionDialog
    - 仅一种就绪时直接导航
    - _需求: 6.1, 6.2, 6.3, 6.4_
  - [x] 6.5 修改 `app/services/text_review_service.py`：支持 material_types 参数
    - 接受 `material_types: list[str]` 参数，仅使用指定材料进行评审
    - 未传 material_types 时默认使用所有已就绪材料（向后兼容）
    - _需求: 2.5, 2.6, 4.6_

- [x] 7. 中文标签显示与子页面导航
  - [x] 7.1 修改 `ProjectList.tsx`：集成 useLabelResolver
    - 项目卡片上的赛事、赛道、组别标签显示中文名称
    - _需求: 7.2_
  - [x] 7.2 修改 `ProjectDashboard.tsx`：集成 useLabelResolver
    - 项目头部的赛事、赛道、组别标签显示中文名称
    - _需求: 7.3_
  - [x] 7.3 为剩余子页面添加 BackButton
...
- [x] 8. 侧边栏项目树
  - [x] 8.1 创建 `frontend/src/components/ProjectTree.tsx` 组件
    - 使用 Ant Design `Tree` 组件
    - 按 competition → track → group 分组，节点名称通过 useLabelResolver 解析
    - 点击叶子节点导航至项目仪表盘
    - 无项目的空节点不显示
    - _需求: 9.1, 9.2, 9.3, 9.6_
  - [x] 8.2 修改 `AppLayout.tsx`：集成 ProjectTree 和 NetworkStatusBar
...
- [x] 9. 检查点 — 确保前端核心功能测试通过
  - 确保所有测试通过，如有问题请向用户确认。

- [x] 10. 进度时间线日期显示
  - [x] 10.1 在 `app/routes/projects.py` 中新增阶段日期查询端点或扩展现有端点
    - 查询 `stage_configs` 表，返回项目所属赛事各阶段日期
    - _需求: 10.4_
  - [x] 10.2 修改 `ProjectDashboard.tsx` 中的 Steps 组件
...
- [x] 11. 会议分享链接（优先级较低，需先集成 GetStream React Video SDK）
  - [x] 11.1 在 `app/services/live_presentation_service.py` 中新增 `generate_share_link` 方法
    - 生成包含 session_id 和认证 token 的 URL
    - 通过 GetStream SDK 的 `create_token` 生成临时访问令牌
    - _需求: 11.4_
  - [x] 11.2 在 `app/routes/live_presentation.py` 中新增分享链接端点
  ...
  - [x] 11.4 修改 `LivePresentation.tsx`：添加分享链接按钮
    - 添加 BackButton（返回项目仪表盘）
    - 会议发起后显示"生成分享链接"按钮
    - 点击后生成 URL 并复制到剪贴板
...
- [x] 12. 材料版本下载
  - [x] 12.1 在 `app/services/material_service.py` 中新增 `get_download_url` 方法
    - 使用 Supabase Storage 的 `create_signed_url` 生成签名下载 URL
    - _需求: 12.3_
  - [x] 12.2 在 `app/routes/materials.py` 中新增 `GET /api/projects/{project_id}/materials/{material_id}/download` 端点
    - 返回 `DownloadUrlResponse`
    - 文件不存在时返回 404
    - _需求: 12.2, 12.3, 12.4_
  - [x] 12.3 修改 `MaterialCenter.tsx`：版本历史弹窗添加下载按钮
    - 添加 BackButton（返回项目仪表盘）
    - 每个版本记录行添加"下载"按钮
...
- [x] 13. AI 项目简介提取
  ...
  - [x] 13.4 修改 `ProjectDashboard.tsx`：添加项目简介卡片
    - 在项目头部区域展示 AI 提取的简介
    - 支持用户编辑和保存
    - 材料更新后提示是否重新提取
    - _需求: 13.3, 13.4, 13.6_
...
- [x] 14. 自定义标签系统
  - [x] 14.1 创建 `app/services/tag_service.py`
    - 实现标签 CRUD 和项目-标签关联管理
    - _需求: 14.1, 14.5, 14.7_
  - [x] 14.2 创建 `app/routes/tags.py` 标签路由
    - `POST/GET/PUT/DELETE /api/tags`：标签 CRUD
    - `POST/DELETE /api/projects/{project_id}/tags`：项目标签关联
    - 标签名称重复时返回 409
    - _需求: 14.1, 14.5, 14.7_
  - [x] 14.3 在 `app/main.py` 中注册标签路由
    - _需求: 14.7_
  - [x] 14.4 在 `frontend/src/services/api.ts` 中新增 `tagApi` 方法集
    - _需求: 14.7_
  - [x] 14.5 修改 `ProjectDashboard.tsx`：添加标签管理功能
    - "添加标签"入口，提供至少 8 种预设颜色
    - 显示已关联标签，支持移除
    - _需求: 14.1, 14.2, 14.3, 14.5_
  - [x] 14.6 修改 `ProjectList.tsx`：添加标签筛选
    - 点击标签筛选仅显示包含该标签的项目
    - _需求: 14.6_
  - [x] 14.7 修改 `ProjectTree.tsx`：项目节点旁显示标签色点
    - _需求: 14.4_
  - [ ]* 14.8 编写属性测试：标签筛选正确性
    - **Property 9: 标签筛选正确性**
    - **验证: 需求 14.6**
...
- [x] 15. 检查点 — 确保所有功能模块测试通过
  - 确保所有测试通过，如有问题请向用户确认。

- [x] 16. 项目报告导出
  - [x] 16.1 创建 `app/services/export_service.py`
    - 使用 `reportlab` 或 `weasyprint` 生成 PDF
    - 包含项目基本信息、材料状态、评审结果摘要、评分汇总
    - 无评审记录时标注"暂无评审记录"
    - _需求: 16.2, 16.3, 16.4_
  - [x] 16.2 在 `app/routes/projects.py` 中新增 `GET /api/projects/{project_id}/export` 端点
  ...
  - [x] 16.4 修改 `ProjectDashboard.tsx`：添加"导出项目报告"按钮
    - 点击后调用导出 API 并触发浏览器下载 PDF
    - _需求: 16.1_

- [ ] 17. 最终集成与路由注册
  - [x] 17.1 在 `app/main.py` 中注册所有新增路由
    - 确保 materials status、name-mappings、profile、tags、share、export 端点均已注册
    - _需求: 全部_
  - [x] 17.2 在 `App.tsx` 中添加会议加入路由
    - 添加 `/live/join/:shareToken` 路由
    - _需求: 11.3_
  - [x] 17.3 在 `AppLayout.tsx` 中集成 `LabelResolverContext` Provider
