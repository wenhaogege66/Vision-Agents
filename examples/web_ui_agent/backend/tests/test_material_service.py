...
class TestProperty8PPTToImageCompleteness:
    """Property 8: PPT转图像完整性

    对于任意上传的PPT文件，转换后生成的图像数量应等于PPT的页数，
    且每张图像文件应为有效的图像格式。

    Feature: competition-judge-system, Property 8: PPT转图像完整性
    Validates: Requirements 3.9
    """

    @given(num_slides=st.integers(min_value=1, max_value=20))
    @h_settings(max_examples=100, deadline=None)
    @pytest.mark.asyncio
    async def test_image_count_equals_slide_count(self, num_slides: int):
        """转换后图像数量应等于PPT页数。"""
        from pptx import Presentation as PptxPresentation
        from app.services.ppt_convert_service import PPTConvertService

        # 动态创建一个包含 num_slides 页的PPT
        ...
        # Mock Supabase: download 返回PPT字节，upload 记录调用
        sb = MagicMock()
        bucket = MagicMock()
        sb.storage.from_.return_value = bucket
        bucket.download.return_value = ppt_bytes

        uploaded_images: list[tuple[str, bytes]] = []

        def capture_upload(path, file, file_options=None):
            uploaded_images.append((path, file))

        bucket.upload.side_effect = capture_upload

        svc = PPTConvertService(sb)
        file_path = "proj-1/text_ppt/v1_slides.pptx"
        image_paths = await svc.convert_to_images(file_path)

        # 属性验证：图像数量 == 幻灯片页数
        assert len(image_paths) == num_slides, (
            f"PPT有 {num_slides} 页，但生成了 {len(image_paths)} 张图像"
        )

        # 属性验证：每张图像都是有效的PNG
        assert len(uploaded_images) == num_slides
        for path, img_bytes in uploaded_images:
            assert path.endswith(".png"), f"图像路径应以 .png 结尾: {path}"
            # PNG 文件头魔数: \x89PNG\r\n\x1a\n
            assert img_bytes[:4] == b"\x89PNG", (
                f"图像 {path} 不是有效的PNG格式"
            )

        # 属性验证：图像路径格式正确
        for i, path in enumerate(image_paths, start=1):
            expected_name = f"page_{i:03d}.png"
            assert expected_name in path, (
                f"第 {i} 张图像路径应包含 {expected_name}，实际为 {path}"
            )
    ...
    @h_settings(max_examples=100, deadline=None)
    @pytest.mark.asyncio
    async def test_each_slide_text_preserved_in_image(self, num_slides: int, data):
        """每页幻灯片的文本内容应被提取并渲染到对应图像中（图像非空）。"""
        from pptx import Presentation as PptxPresentation
        from app.services.ppt_convert_service import PPTConvertService
        ...
        slide_texts = []
        for i in range(num_slides):
            text = data.draw(st.text(min_size=1, max_size=30, alphabet=st.characters(
        ...
        bucket.upload.side_effect = lambda path, file, **kw: uploaded_images.append(file)

        svc = PPTConvertService(sb)
        image_paths = await svc.convert_to_images("proj-1/text_ppt/v1_test.pptx")

        assert len(image_paths) == num_slides

        # 每张图像应有非零大小（内容被渲染）
        for i, img_bytes in enumerate(uploaded_images):
            assert len(img_bytes) > 0, f"第 {i+1} 张图像为空"
            # 验证是有效的 PIL Image
