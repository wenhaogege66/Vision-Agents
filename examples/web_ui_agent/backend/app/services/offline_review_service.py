"""离线路演评审服务：获取路演PPT+视频→加载规则→组装Prompt→调用AI→解析结果→存储。

基于多模态AI（通义千问）分析路演视频和路演PPT图像，
按照赛事/赛道/组别的评审规则生成综合评审报告，
包含演讲表现评价、PPT内容评价、综合评分和改进建议。
"""
...
class OfflineReviewService:
    ...
    async def review(
        ...
        Raises:
            HTTPException(400): 路演PPT或路演视频未上传
        ...
        material_content = (
            ...
            f"路演视频文件: {presentation_video['file_name']} "
            f"(版本 {presentation_video['version']})"
        )
        ...
        user_content.append(
            {
                ...
                "text": (
                    "请结合路演视频和路演PPT进行综合评审。"
                    "评审报告需包含：演讲表现评价、PPT内容评价、综合评分和改进建议。"
                ),
            }
        )
        ...
        material_versions: dict = {
            ...
            "presentation_video": presentation_video["version"],
        }
